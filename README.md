...
* The *connector code* will be in a Github repository (public or private).
  * *connector code* acts as the interface between the inputs obtained from the web interface (as a JSON file) and the outputs generated by your model/code also expressed as a JSON file that are displayed by the web interface. Both the inputs and outpus have to conform to input or output JSON schema respectively.
...
1. Create a [Dockerfile](https://docs.docker.com/engine/reference/builder/), installing all the necessary dependencies to run your model/code in the docker instance, and ensure that your model/code can run in this docker instance.
...
1. Develop your connector (iteratively):
   ...
   1. Edit [`Dockerfile`](Dockerfile) to create an image containing both your model and connector code, with any dependencies, as described in more detail below.
...
- The command must return **MA: is this something the code or the docker instance needs to do?**:
...
- At run-time, 
  - Copy/store any input data into `/data/input/inputFile.json` **MA: is this not a file you read in and hence you will not store anything in it but will read from it?**
    - `/data/input/` will be volume mounted into the container, and will be available for download in the UI after completion **MA: Surely this should be at the start - no point after completion!!**
